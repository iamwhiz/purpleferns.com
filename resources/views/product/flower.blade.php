...
@section('content')
   ...
   <!-- <img src="{{url('/')}}/assets/images/flowers_b.jpg"> -->
    <?php if(empty($category->banner_image)) {  ?>		
   <img src="{{url('/')}}/public/assets/images/flowers_b.jpg">
    <?php }  else { ?>
	...
	<div class="container">
		<div class="inner_breadcums">
			<div class="inner_left">
               {!! Breadcrumbs::render('flower',$catname) !!}
				<!-- <ul>
				  <li><a href="#">Home</a></li>
			...
			<li><span>Showing {{ $products->lastItem() }} out of  {{$products->total()}} Gifts
				   		 ...
				   		 <input type="hidden" name="type" value="flowers" id="type">
						...
						<div class="col-xs-12 col-sm-3 prdct_box">
							...
							<div class="wk_prdct_inner">
								<div class="wk_prdct_img">
									<a href="{{url('/')}}/flower/{{$product->slug}}"><img src="<?php echo $image->productMainImage($product->id); ?>" > </a>
								</div>
								<div class="wk_prdct_text">
									<div class="hover_ul">
										...
										@if(Auth::check())
										  ...
										  @endif
										  <!-- <li><a href="{{url('/')}}/flower/{{$product->slug}}">
									...
									<h3 title="{{$product->name}}"><a href="{{url('/')}}/flower/{{$product->slug}}">{{$product->name}}</a></h3>
