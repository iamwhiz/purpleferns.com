...
@section('content')
   ...
   <img src="{{url('/')}}/public/assets/images/gifts_b.jpg" alt="Banner Image">
						...
						<div class="flower_box">
								<div class="cake_img flower_imgs">
                                   <?php if(!empty($product->cat_image)) {   ?>
                                   	<a href="{{url('/')}}/gifts/{{$product->cat_slug}}">
								...
								<div class="flower_heading"><h3><a href="{{url('/')}}/gifts/{{$product->cat_slug}}">{{$product->cat_name}}</a></h3></div>
			            </div>
						 @endforeach
